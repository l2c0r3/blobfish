...
Um dies möglichst einfach zu gestalten, wurden die Klassen \textit{EvaluationUtil} sowie \textit{PowerSetUtil} entworfen.

\textit{EvaluationUtil} enthält eine Methode, welche alle mögliche Evaluationmöglichkeiten zurückgeben soll. \\
Dafür wird zuerst mittels \href{https://github.com/classgraph/classgraph}{classgraph}, eine Java-Bibliothek alle Klassen, welche
Interface \textit{EvalStrategy} implementieren, gescannt und ein Objekt davon initialisiert.

Nun, da wir die einzelnen Evaluationsmöglichkeiten haben, möchten wir diese auch miteinander kombinieren. Hierfür wurde die
Klasse \textit{CompositeEvalStrategy} geschrieben wurde.\\
...
Die Potenzmenge enthält nämlich alle Teilmengen einer Menge - also alle Kombinationen in aller möglichen Grössen sowie die einzelnen Evaluationsklassen (\cite{potenzmenge}):

So wird aus einer List von $\{\texttt{a}, \texttt{b}, \texttt{c}\}$ folgendes zurückgegeben:
...
Die Funktionalität, die Pozenzmenge einer Liste von Objekte zu ermitteln, wurde in die Klasse \textit{PowerSetUtil} ausgelagert.
