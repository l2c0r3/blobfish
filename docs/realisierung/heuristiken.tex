...
Genau so existieren auch mehrere \gls{glos:psqt}s.
Zunächst sollten die Stockfish \gls{glos:psqt}s verwendet werden (\cite{piecesquaretablelichessstockfish}), jedoch wurde festgestellt, dass Stockfish nicht die gleichen Basiswerte der Figuren verwendet (\cite{piecevalueslichessstockfish}).
...
Schlussendlich wurden die \gls{glos:psqt}s von \href{https://www.chessprogramming.org/Simplified_Evaluation_Function}{Chess Programming Wiki}~verwendet (\cite{piecesquaretablesef}).
Dementsprechend wurden für den König zwei \gls{glos:psqt}s erstellt.
...
Die Implementation der \gls{glos:psqt} Evaluation ist ziemlich einfach.
\lstinputlisting[
    firstline=14,
    lastline=42,
    firstnumber=14,
...
Folglich wird durch das ganze Brett iteriert und überall wo sich eine Figur befindet, wird dessen Wert aus der entsprechenden \gls{glos:psqt} ausgelesen.
Wie mit den anderen Evaluationen werden Werte für den schwarzen Spieler negiert.
Alle Werte werden am Schluss aufsummiert.

Die Arrays für die \gls{glos:psqt}s wurden so strukturiert, dass der Zuschauer die Perspektive seines Schachbretts hat.
...
\lstinputlisting[
    ...
    lastline=90,
    firstnumber=81,
    caption={Code - PSQT für den Turm},
    label={lst:psqt-fuer-turm}
]{../src/main/java/ch/hslu/cas/msed/blobfish/eval/PieceSquareEval.java}
...
In dieser Abbildung stellt das Schachbrett den Aufbau der \gls{glos:psqt}s dar.
Die Nummern in den Feldern sind die Indexe für den jeweiligen Spieler um den Wert aus einer \gls{glos:psqt} für das entsprechende Feld zu holen.
...
Der benötigte Index um den korrekten Wert aus der \gls{glos:psqt} auszulesen ist 49.
Die entsprechende \gls{glos:psqt} liefert gemäss Code-Ausschnitt~\ref{lst:psqt-fuer-turm} den Wert 0.
